...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const runs = await articleInspectionService.enqueueDailyIncrementalRun(params.limit)
    ...
    console.error('Failed to execute scheduled inspection cycle:', error)
    return Response.json(
      errorResponse(ErrorCodes.INTERNAL_ERROR, '执行定时巡检失败'),
      { status: 500 }
    )
  }
}
