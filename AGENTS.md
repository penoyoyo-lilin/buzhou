...
36. **提交远程仓库前排除本地测试与开发数据**：后续提交到远程仓库时，默认排除 `tests/**` 和本地数据库文件 `prisma/prisma/data/buzhou.db`，避免将本地测试脚本与开发态 SQLite 数据一并推送。
37. **后台运维动作优先提供批量入口**：像巡检处理、重试、修复这类后台运维操作，若存在积压队列或批量处理场景，默认优先设计批量执行入口，不只提供单条按钮，避免人工逐条处理。
38. **早期后台运维优先减少动作面**：如果现有后台动作可以在同一次请求内完成“入队 + 执行”的最小闭环，则优先复用该动作，不额外增加新的定时器、批量入口或页面按钮，避免运维面继续膨胀。
